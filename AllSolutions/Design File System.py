...
Implement the FileSystem class:
    ...
    bool createPath(string path, int value) Creates a new path and associates a value to it if possible and returns true. Returns false if
...
# easy , don't need Trie, use string operations to find parent
    ...
    def createPath(self, path: str, value: int) -> bool:
        ...
        for idx in range(len(path))[::-1]:
            if path[idx] == '/':
                break
        parent = path[:idx]
        if parent == '' or parent in self.path_val:
