...
所有字典中的字母。 应该是leetcode的原题了了。 做法是two pointer + hash
"""
...
def find_min_window(S, T):     
    left, right = 0, 0     
    c = defaultdict(int)    
    for ch in T: # hashmap allows for O(1) search    
        c[ch] += 1     
    min_str = None          
    while right <= len(S):         
        if all(map(lambda x: True if x <= 0 else False, c.values())): # all elements in dic have been visited           
            if min_str is None or right-left <= len(min_str): # if the first str or shorter than existing str
